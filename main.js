...
let forSaleContentContainer = document.querySelector(".for-sale-content-container");
let forRentContentContainer = document.querySelector(".for-rent-content-container");
...
if (forSaleContentContainer) {
  for (let i = 0; i < listings.length; i++) {
    if (listings[i].category === "Satılık") {
      let newCard = createContentItemCard(listings[i]);
      forSaleContentContainer.appendChild(newCard);
    }
  }
} else {
  console.error("forSaleContentContainer not found in the DOM.");
}


if (forRentContentContainer) {
  for (let i = 0; i < listings.length; i++) {
    if (listings[i].category === "Kiralık") {
      let newCard = createContentItemCard(listings[i]);
      forRentContentContainer.appendChild(newCard);
    }
  }
} else {
